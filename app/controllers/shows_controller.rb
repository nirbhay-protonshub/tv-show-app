class ShowsController < ApplicationController
  ...
  def set_as_favourite
    favourite = current_user.favourites.create(show_id: params[:show_id])
    time = Time.now + (favourite.show.time.to_i*60*60) - 30.minutes
    EmailWorker.perform_at(time, "send_notification", favourite.show.id, current_user.id)
    redirect_to shows_path
  end

  def remove_from_favourites
    favourite = current_user.favourites.find_by(show_id: params[:show_id])
    favourite.destroy
    redirect_to shows_path
  end
